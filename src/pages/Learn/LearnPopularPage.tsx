// src/pages/Learn/LearnPopularPage.tsx
...
export default function LearnPopularPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[LearnPopularPage] fetch error:", e);
        if (!alive) return;
        setErrorMsg("인기 코스를 불러오지 못했어요.");
        setCourses([]);
      } finally {
        if (!alive) return;
        setLoading(false);
      ...
      <div className={page.container}>
        <header className={page.header}>
          <img
            ...
            onClick={() => navigate(-1)}
          />
          <h1 className={page.title}>인기 코스</h1>
        </header>
