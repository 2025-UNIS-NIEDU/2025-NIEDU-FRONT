// src/pages/Learn/LearnPersonalizedPage.tsx
...
export default function LearnPersonalizedPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[LearnPersonalizedPage] fetch error:", e);
        if (!alive) return;
        setErrorMsg("맞춤추천 코스를 불러오지 못했어요.");
        setCourses([]);
      } finally {
        if (!alive) return;
        setLoading(false);
      ...
      <div className={page.container}>
        <header className={page.header}>
          <img
            ...
            onClick={() => navigate(-1)}
          />
          <h1 className={page.title}>맞춤추천 코스</h1>
        </header>
            ...
            {loading && courses.length === 0 ? (
              ...
              courses.map((c) => (
                <div
                  ...
                  onClick={() => goToDetail(String(c.courseId), { from: "learn-custom" })}
                >
                  <img
