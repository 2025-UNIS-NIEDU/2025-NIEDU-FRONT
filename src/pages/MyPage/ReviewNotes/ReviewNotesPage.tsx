...
export default function ReviewNotesPage() {
  ...
  // ✅ date는 optional (Swagger: 초기 진입은 date 없이)
  ...
  useEffect(() => {
    const run = async () => {
      setLoading(true);
      try {
        // ✅ date 없으면 /api/my/review-notes 그대로 호출
        const qs = new URLSearchParams();
        if (date) qs.set("date", date);
        if (sessionId) qs.set("sessionId", sessionId);

        const url = qs.toString()
          ? `/api/my/review-notes?${qs.toString()}`
          : `/api/my/review-notes`;
        ...
        const notesRaw =
          Array.isArray(d?.reviewNotes)
            ? d.reviewNotes
            : Array.isArray(d?.quizzes)
              ? d.quizzes
              : [];
